          ...
          <PopoverBody>
            <Col className="text-start">
              <Button
                ...
                onClick={() => {
                  props.setActionMessage({ actionType: "Pin", item: item });
                  props.onOpen();
                }}
              >
              ...
              {item?.user?.id == props.context.userLens.profile.id ? (
                <>
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      props.handleEditing(item);
                    }}
                  >
                    <Row className="hr-between w-100">Edit</Row>
                  </Button>
                  <Button
                    variant="transparent"
                    size="sm"
                    onClick={() => {
                      props.setActionMessage({
                        actionType: "Delete",
                        item: item,
                      });
                      props.onOpen();
          ...
          <Row key={index}>
            {Object.keys(props.reactions).map((reaction, index) => (
              <Badge
                key={index}
                variant={
                  props.reactions[reaction]["clicked"] ? `subtle` : `outline`
                }
                className="m-r-0-5"
          ...
          <Conversation key={props.index}>
            <Row className="message">
              <Avatar
                ...
                className="m-r-1"
                onClick={() => {
                  props.setMember(userHook);
                }}
              >
                {props.item.user.online ? (
              ...
              <Col className="w-100">
                <Row
                  className="hr-between"
                  onClick={() => {
                    props.setMember(userHook);
                  }}
                >
                  <Heading as="h5" size="xs" className="m-b-0-5">
                    {userHook?.name}
                  </Heading>
              ...
              <Row className="m-b-0-5 w-100">
                {props.editingId && props.editingId == props.item.id ? (
                  <Col className="w-100 vr-center">
                    <Textarea
                      onChange={e => {
                        props.setEditingMessage(e.currentTarget.value);
                      }}
                      ref={textareaRef}
