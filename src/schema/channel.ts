...
export class Channel$ {
  ...
  constructor(source: string = "", data: any = null) {
    this.source = source;
    switch (this.source) {
      case "xmtp":
  ...
  setFromStream(data: any) {
    ...
    (this.type = data.data?.type),
      ...
      (this.lastMessage =
        data?.state?.messageSets[0]?.messages[
          data?.state?.messageSets[0]?.messages?.length - 1
        ]),
  ...
  setFromXmtp(data: any) {
    ...
    this.name =
      data.peer?.lens?.name ||
      data.peer?.lens?.handle ||
      data.peer?.lens?.ownedBy ||
      data?.peerAddress;
    this.orgId = data?.orgId;
    this.private = data?.private;
    this.users = data?.users;
    ...
    this.permissions = data?.permissions;
    this.image =
      helperIPFS(data?.peer?.lens?.picture?.original?.url) ||
      helperIPFS(data?.peer?.lens?.image);
    this.lastMessage = "";
    this.membersCount = 0;
  ...
  updatePeerLens(user: any) {
    ...
    this.name =
      this.peer?.lens?.name ||
      this.peer?.lens?.handle ||
      this.peer?.lens?.ownedBy ||
      this?.peerAddress;
  }
}
